...
const closeUserCard = () => {
  isActiveUserCard.value = false;
  isActiveUserCardSide.value = false;
};

const handleClickOutside = (event: any) => {
  const userCard = document.querySelector(".user-card"); // Adjust the selector as per your actual CSS
...
onMounted(() => {
  ...
  window.addEventListener('resize', checkWindowWidth); // window resize listen
  document.addEventListener('click', handleClickOutside);
});

onBeforeUnmount(() => {
  window.removeEventListener('resize', checkWindowWidth); // Remove listener
...
<template >
  <nav class="bg-background flex fixed z-20 pb-[15px] w-full items-center py-[10px] px-[20px]">
    ...
    <!-- web navbar  -->
    <div v-if="isActive"
      class='ml-auto relative flex-shrink-0 space-x-[10px] md:space-x-[15px] flex items-center justify-center'>
      <button>
      ...
      <!-- User Image -->
      <button @click.stop @click="toggleUserCard"><img class="rounded-[9px]" src="/assets/icons/user.png" alt="user"></button>
      ...
      <Transition name="fade-up">
        <UserCard v-show="isActiveUserCard" class="user-card absolute top-[65px]" />
      </Transition>
    </div>
    ...
    <Transition>
      <aside
        class="z-20 sm:hidden rounded-b-md h-fit w-full absolute top-0 right-0 border-b border-[#F1F1F2] bg-background py-2 ">
        <div class="relative">
          <div class="flex flex-row gap-y-[10px] px-3 items-center justify-around">
            ...
            <button><img class="bg-white cursor-pointer rounded-[9px] p-[10px]" src="/assets/icons/notification-2.svg"
                alt="logo"></button>
            <button @click.stop @click="toggleUserCardSide"><img class="rounded-[9px]" cursor-pointer src="/assets/icons/user.png"
                alt="user"></button>
            <!-- User Card  -->
            <Transition name="fade-up">
              <UserCard v-show="isActiveUserCardSide" class="user-card absolute top-[65px] right-[68px]" />
            </Transition>
